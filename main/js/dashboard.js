...
document.addEventListener("DOMContentLoaded", function () {
  ...
  recipeCards.forEach((card) => {
    ...
    card.addEventListener("click", function (e) {
      if (
        !e.target.closest(".delete-recipe") &&
    ...
    fetch(`add-recipe.php?id=${recipeId}`)
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          alert(
            "Error loading recipe details: " + (data.message || "Unknown error")
          );
        }
      })
      .catch((error) => {
  ...
  function populateRecipeModal(recipe) {
    ...
    document.getElementById("modal-prep-time").textContent =
      (recipe.prep_time || "N/A") + " minutes";
    document.getElementById("modal-calories").textContent = recipe.calories
      ? recipe.calories + " calories"
      : "Not specified";
    document.getElementById("modal-servings").textContent =
    ...
    document.getElementById("modal-carbs").textContent = recipe.carbs
      ? recipe.carbs + " g"
      : "Not specified";
    document.getElementById("modal-fiber").textContent = recipe.fiber
      ? recipe.fiber + " g"
      : "Not specified";
  ...
  function updateShoppingListCount() {
    fetch("shopping-list.php?get_count=1")
      ...
      .then((data) => {
        if (data.success) {
          document.querySelector(".shopping-list-count").textContent =
            data.count;
        }
      })
      .catch((error) => {
